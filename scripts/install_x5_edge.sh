...
CONSOLE_USER="${SAFEEXEC_CONSOLE_USER:-safeexec-console}"
...
create_service_user "${CONSOLE_USER}"

install -d -m 0755 "${SAFEEXEC_ROOT}/.run" "${SAFEEXEC_ROOT}/.run/x5"
for required in "${PRIVATE_KEY}" "${WORK_ORDER_PRIVATE_KEY}" "${WORK_ORDER_PUBLIC_KEY}" "${ENV_FILE}"; do
  if [[ ! -f "${required}" ]]; then
...
chown root:"${CONSOLE_USER}" "${WORK_ORDER_PRIVATE_KEY}"
chmod 0640 "${WORK_ORDER_PRIVATE_KEY}"
...
DASHBOARD_ENV="${RUNTIME_ENV_DIR}/dashboard.env"
